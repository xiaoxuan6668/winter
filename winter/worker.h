#ifndef __WINTER_WORKER_H__
#define __WINTER_WORKER_H__
...
public:
    ...
    static WorkerGroup::ptr Create(uint32_t batch_size, winter::Scheduler* s = winter::Scheduler::GetThis()) {
        return std::make_shared<WorkerGroup>(batch_size, s);
    }

    WorkerGroup(uint32_t batch_size, winter::Scheduler* s = winter::Scheduler::GetThis());
    ~WorkerGroup();
...
public:
    ...
    void schedule(const std::string& name, FiberOrCb fc, int thread = -1) {
        ...
        if(s) {
            ...
            static winter::Logger::ptr s_logger = WINTER_LOG_NAME("system");
            WINTER_LOG_ERROR(s_logger) << "schedule name=" << name
                << " not exists";
        }
    }
...
typedef winter::Singleton<WorkerManager> WorkerMgr;
