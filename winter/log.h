...
#define WINTER_LOG_LEVEL(logger, level) \
    winter::LogEventWrap(winter::LogEvent::ptr(new winter::LogEvent(logger, level, \
                ...
                winter::GetFiberId(), time(0), winter::Thread::GetName()))).getSS()
...
#define WINTER_LOG_FMT_LEVEL(logger, level, fmt, ...) \
    if(logger->getLevel() <= level) \
        winter::LogEventWrap(winter::LogEvent::ptr(new winter::LogEvent(logger, level, \
                        __FILE__, __LINE__, 0, winter::GetThreadId(),\
                winter::GetFiberId(), time(0), winter::Thread::GetName()))).getEvent()->format(fmt, __VA_ARGS__)
...
#define WINTER_LOG_NAME(name) winter::LoggerMgr::GetInstance()->getLogger(name)
