...
HttpConnection::~HttpConnection() {
    WINTER_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string& uri
    ...
    if(!turi) {
        WINTER_LOG_ERROR(g_logger) << "invalid uri=" << uri;
    }
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    uint64_t now_ms = winter::GetCurrentMS();
    ...
    if(!ptr) {
        ...
        if(!addr) {
            WINTER_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if(!sock) {
            WINTER_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr)) {
            WINTER_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if(!ptr->isConnected()
            || ((ptr->m_createTime + pool->m_maxAliveTime) >= winter::GetCurrentMS())
