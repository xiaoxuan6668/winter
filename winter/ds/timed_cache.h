#ifndef __WINTER_DS_TIMED_CACHE_H__
#define __WINTER_DS_TIMED_CACHE_H__
...
template<class K, class V, class RWMutexType = winter::RWMutex>
class TimedCache {
private:
    struct Item {
...
public:
    ...
    void set(const K& k, const V& v, uint64_t expired) {
        ...
        auto sit = m_timed.insert(Item(k, v, expired + winter::GetCurrentMS()));
    ...
    bool expired(const K& k, const uint64_t& ts) {
        ...
        uint64_t tts = ts + winter::GetCurrentMS();
        if(it->second->ts == tts) {
            return true;
        }
        auto item = *it->second;
...
template<class K, class V, class RWMutexType = winter::RWMutex, class Hash = std::hash<K> >
