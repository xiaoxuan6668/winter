#ifndef __WINTER_DS_TIMED_LRU_CACHE_H__
#define __WINTER_DS_TIMED_LRU_CACHE_H__
...
template<class K, class V, class MutexType = winter::Mutex>
class TimedLruCache {
private:
    struct Item {
...
public:
    ...
    void set(const K& k, const V& v, uint64_t expired) {
        ...
        if(it != m_cache.end()) {
            ...
            it->second->val = v;
            it->second->ts = expired + winter::GetCurrentMS();
            m_timed.insert(it->second);
            return;
        }

        m_keys.emplace_front(Item(k, v, expired + winter::GetCurrentMS()));
    ...
    size_t checkTimeout(const uint64_t& ts = winter::GetCurrentMS()) {
        size_t size = 0;
        typename MutexType::Lock lock(m_mutex);
...
template<class K, class V, class MutexType = winter::Mutex, class Hash = std::hash<K> >
