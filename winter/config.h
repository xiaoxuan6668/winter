 ...
 * @brief 配置模块
 */
#ifndef __WINTER_CONFIG_H__
#define __WINTER_CONFIG_H__
...
public:
    ...
    bool fromString(const std::string& val) override {
        try {
            ...
            WINTER_LOG_ERROR(WINTER_LOG_ROOT()) << "ConfigVar::fromString exception "
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,
        ...
        if(it != GetDatas().end()) {
            ...
            if(tmp) {
                WINTER_LOG_INFO(WINTER_LOG_ROOT()) << "Lookup name=" << name << " exists";
                return tmp;
            } else {
                WINTER_LOG_ERROR(WINTER_LOG_ROOT()) << "Lookup name=" << name << " exists but type not "
        ...
        if(name.find_first_not_of("abcdefghikjlmnopqrstuvwxyz._012345678")
                != std::string::npos) {
            WINTER_LOG_ERROR(WINTER_LOG_ROOT()) << "Lookup name invalid " << name;
