...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    if(fd_ctx->events & event) {
        ...
        WINTER_ASSERT(!(fd_ctx->events & event));
    }
    ...
    if(rt) {
        WINTER_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", "
...
void IOManager::idle() {
    ...
    while(true) {
        ...
        listExpiredCb(cbs);
        if(!cbs.empty()) {
            //WINTER_LOG_DEBUG(g_logger) << "on timer cbs.size=" << cbs.size();
