...
int ZlibStream::init(Type type, int level, int window_bits
    ...
    WINTER_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    WINTER_ASSERT((window_bits >= 8 && window_bits <= 15));
    WINTER_ASSERT((memlevel >= 1 && memlevel <= 9));
