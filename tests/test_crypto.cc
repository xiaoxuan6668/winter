...
void test_aes256ecb() {
    for(int i = 0; i < 4097; ++i) {
        auto str = winter::random_string(i);
        std::string encode;
        encode.resize(i+ 30);
        std::string decode;
        decode.resize(i+ 30);

        auto key = winter::random_string(32);

        int32_t len = winter::CryptoUtil::AES256Ecb(key.c_str(), str.c_str(), str.size(), &encode[0], true);
        std::cout << "encode_len: " << len << std::endl;
        if(len > 0) {
            encode.resize(len);
        }
        len = winter::CryptoUtil::AES256Ecb(key.c_str(), encode.c_str(), encode.size(), &decode[0], false);
...
void test_aes256cbc() {
    for(int i = 0; i < 4097; ++i) {
        ...
        auto key = winter::random_string(32);
        auto iv = winter::random_string(16);

        int32_t len = winter::CryptoUtil::AES256Cbc(key.c_str(), iv.c_str(), str.c_str(), str.size(), &encode[0], true);
        std::cout << "encode_len: " << len << std::endl;
        if(len > 0) {
            encode.resize(len);
        }
        len = winter::CryptoUtil::AES256Cbc(key.c_str(), iv.c_str(), encode.c_str(), encode.size(), &decode[0], false);
        std::cout << "decode_len: " << len << std::endl;
        if(len > 0) {
            decode.resize(len);
        }
...
void test_one() {
    //std::string key = winter::random_string(32);
    //std::string iv = winter::random_string(16);
    //std::string str = winter::random_string(125);
    ...
    int32_t len = winter::CryptoUtil::AES128Cbc(key.c_str(), iv.c_str(), str.c_str(), str.size(), &encode[0], true);
    if(len >= 0) {
        encode.resize(len);
    }

    int32_t len2 = winter::CryptoUtil::AES128Cbc(key.c_str(), iv.c_str(), encode.c_str(), encode.size(), &decode[0], false);
    if(len2 >= 0) {
        decode.resize(len2);
    }
    ...
    std::cout << "encode: " << winter::base64encode(encode) << " - len=" << encode.size() << std::endl;
...
void test_rsa() {
    ...
    //std::cout << "generate: " << winter::RSACipher::GenerateKey(pubkey_path, prikey_path) << std::endl;
    winter::RSACipher::ptr rsa = winter::RSACipher::Create(pubkey_path, prikey_path);
    if(!rsa) {
